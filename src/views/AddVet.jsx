...
const AddVet = () => {
  ...
  const [veterinariaTmp, setVeterinariaTmp] = useState({});
  ...
  const getVeterinaries = async () => {
    try {
      const response = await clienteAxios.get(
        "https://www.inegi.org.mx/app/api/denue/v1/consulta/BuscarEntidad/Veterinaria/23/1/1000/dcd3720b-ed53-44d8-9630-da796b73fe3f"
      );
  ...
  useEffect(() => {
    if (selectVet) {
      const veterinaria = veterinaries[0].find((vet) =>
        selectVet == vet.CLEE ? vet : null
      );
      setVeterinariaTmp(veterinaria);
      ...
      setLatitude({ value: veterinaria.Latitude, error: "" });
      setLongitude({ value: veterinaria.Longitude, error: "" });
    }
  }, [selectVet]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      //      setLoader(false);
      //  }
      console.log(veterinariaTmp);
      const { data } = await clienteAxios.post(
        "/veterinary/create",
        {
          clee: veterinariaTmp.CLEE || selectVet,
          name: veterinariaTmp.Nombre || name.value,
          business_name: '',
          class_activity: veterinariaTmp.Clase_actividad || activityClass.value,
          email: veterinariaTmp.Correo_e || email.value,
          street: veterinariaTmp.Calle || street.value,
          no_ext: veterinariaTmp.Num_Exterior || externalNumber.value,
          no_int: veterinariaTmp.Num_Interior || internalNumber.value,
          colony: veterinariaTmp.Colonia || colony.value,
          code_postal: veterinariaTmp.CP || postalCode.value,
          location: veterinariaTmp.Ubicacion || location.value,
          phone_number: telephone.value || veterinariaTmp.Telefono,
          website: '' || website.value,
          longitude: veterinariaTmp.Longitud || longitude.value,
          latitude: veterinariaTmp.Latitud || latitude.value,
          store_number: veterinariaTmp.numero_local || storeNumber.value,
        },
        getConfig()
      );
      setLoader(false);
      ...
      if (error.response.data.status === 400) {
        ...
        setBusinessName({
          ...businessName,
          error: error.response.data.errors.businessName,
        });
        setActClass({
          ...activityClass,
          error: error.response.data.errors.activityClass,
        });
        setPostalCode({
          ...postalCode,
          error: error.response.data.errors.postalCode,
        });
        setLocation({
          ...location,
          error: error.response.data.errors.location,
        });
        setColony({ ...colony, error: error.response.data.errors.colony });
        setStreet({ ...street, error: error.response.data.errors.street });
        setExtNumber({
          ...externalNumber,
          error: error.response.data.errors.externalNumber,
        });
        setIntNumber({
          ...internalNumber,
          error: error.response.data.errors.internalNumber,
        });
        setTelephone({
          ...telephone,
          error: error.response.data.errors.telephone,
        });
        setEmail({ ...email, error: error.response.data.errors.email });
        setWebSite({ ...website, error: error.response.data.errors.website });
        setLatitude({
          ...latitude,
          error: error.response.data.errors.latitude,
        });
        setLongitude({
          ...longitude,
          error: error.response.data.errors.longitude,
        });
        setStoreNumber({
          ...storeNumber,
          error: error.response.data.errors.storeNumber,
        });
      ...
      <div className="flex justify-center items-center ">
        <div className="w-9/12 sm:w-9/12 md:w-9/12 lg:w-6/12 bg-white shadow border rounded-xl px-4 py-5 my-5">
          <h1 className="text-center text-2xl font-bold text-orange-500">
            AGREGA TU VETERINARIA
          </h1>

          {msgError ? (
            <p className="w-full py-2 px-3 bg-red-600 text-white text-center">
              {msgError}
            </p>
          ) : null}

          <form
            className="py-5 grid  grid-cols-1 md:grid-cols-2 gap-4"
            method="post"
            onSubmit={handleSubmit}
          >
            <div className="flex flex-col justify-center col-span-1 md:col-span-2 lg:col-span-2">
              <label className="flex-1 font-bold block ml-2  text-sm">
                Seleccione el nombre de la veterinaria
              </label>
              ...
              >
                ...
                {veterinaries.length === 0 ? (
                  ...
                  veterinaries[0].map((veterinaria) => (
                    <option
                      key={veterinaria.CLEE}
                      value={veterinaria.CLEE}
            ...
            <div className="flex flex-col justify-center col-span-1 md:col-span-2 lg:col-span-2">
              ...
              {name.error ? <p>{name.error}</p> : null}
            </div>
            ...
            <div className="flex flex-col justify-center">
              ...
              />
              {postalCode.error ? (
                <p className="ml-2 text-xs text-red-500 mt-2">
                  {postalCode.error}
                </p>
              ) : null}
            </div>
            ...
            <div className="flex flex-col justify-center">
              ...
              <input
                ...
                value={colony.value}
                onChange={(e) =>
                  setColony({ value: e.target.value, error: "" })
                }
              />
              {colony.error ? <p>{colony.error}</p> : null}
            </div>
            ...
            <div className="flex flex-col justify-center">
              ...
              <input
                ...
                value={street.value}
                onChange={(e) =>
                  setStreet({ value: e.target.value, error: "" })
                }
              />
              {street.error ? <p>{street.error}</p> : null}
            </div>
            ...
            <div className="flex flex-col justify-center">
              ...
              <input
                ...
                onChange={(e) =>
                  setExtNumber({ value: e.target.value, error: "" })
                }
              />
              {externalNumber.error ? <p>{externalNumber.error}</p> : null}
            </div>
            ...
            <div className="flex flex-col justify-center">
              ...
              <input
                ...
                onChange={(e) =>
                  setTelephone({ value: e.target.value, error: "" })
                }
              />
              {telephone.error ? <p>{telephone.error}</p> : null}
            </div>
            ...
            <div className="flex justify-center items-center col-span-1 md:col-span-2 lg:col-span-2">
              <button
                type="submit"
                className="focus:outline-none hover:bg-orange-700 transition-all duration-300 bg-orange-500  px-8 py-3 rounded-xl w-7/12  md:w-5/12 lg:w-6/12 xl:w-5/12  text-white font-bold"
              >
                Agregar Veterinaria
              </button>
            </div>
